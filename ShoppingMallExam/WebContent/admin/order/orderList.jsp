...
function go_order_save() {
    ...
    if (count == 0) {
      ...
      document.frm.action = "CSCServlet?command=admin_order_save";
      document.frm.submit();
    }
  }
</script>
      ...
      <tr>
          <td>
              <c:choose>
		        ...
		        <span style="font-weight: bold; color: blue">${orderVO.obseq}</span>
		        (<input type="checkbox" name="result" value="${orderVO.obseq}"> 미처리)
		        </c:when>
		        <c:otherwise>
		          <span style="font-weight: bold; color: red">${orderVO.obseq}</span>
