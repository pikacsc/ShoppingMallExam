function go_save(){
	if(document.formm.id.value == ""){
		...
		document.formm.action = "CSCServlet?command=join";
...
function idcheck(){
	...
	var url = "CSCServlet?command=id_check_form&id="
...
function post_zip(){
	var url = "CSCServlet?command=find_zip_num";
	window.open(url,"_blank_1","toolbar=no, menubar=no, scrollbars=yes, "
...
function go_next(){
	if(document.formm.okon1[0].checked == true){
		document.formm.action = "CSCServlet?command=join_form";
		document.formm.submit();
